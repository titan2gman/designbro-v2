...
RSpec.feature 'Ability to create packaging project', js: true do
  describe 'guest' do
    describe 'can create `bottle` project' do
      it 'via filling form' do
        ...
        find('#bottle-packaging-type').click

        expect(page).to have_content('Bottle measurements')

        ['label-width', 'label-height'].each do |field|
          fill_in "bottle-#{field}", with: '1.5 cm'
        end
        ...
        expect(measurements.packaging_type).to eq(:bottle)
    ...
    [:waiting_for_style_details, :waiting_for_finish_details, :waiting_for_audience_details].each do |state|
      it "can visit current page of project that was created before (state: #{state})" do
        project = create(:project, state: state, project_type: :packaging, client: nil)
        create(:bottle_packaging_measurements, project: project)
      ...
      it "can see first page of project that was created before (state: #{state})" do
        project = create(:project, state: state, project_type: :packaging, client: nil)
        measurements = create(:bottle_packaging_measurements, project: project)
  ...
  describe 'client' do
    describe 'can create `bottle` project' do
      it 'via filling form' do
        ...
        ['label-height', 'label-width'].each do |field|
          fill_in "bottle-#{field}", with: '1.5 cm'
        end
