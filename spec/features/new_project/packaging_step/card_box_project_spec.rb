...
RSpec.feature 'Ability to create packaging project', js: true do
  describe 'guest' do
    describe 'can create `card-box` project' do
      it 'via filling form' do
        ...
        find('#card-box-packaging-type').click

        expect(page).to have_content('Cardboard Box measurements')

        ['front-width', 'front-height', 'side-depth'].each do |field|
          fill_in "card-box-#{field}", with: '1.5 cm'
        end
        ...
        expect(measurements.packaging_type).to eq :card_box

        [:front_height, :front_width, :side_depth].each do |property|
    ...
    [:waiting_for_style_details, :waiting_for_finish_details, :waiting_for_audience_details].each do |state|
      it "can see current page of project that was created before (state: #{state})" do
        ...
        create(:card_box_packaging_measurements, project: project)
      ...
      it "can see first page of project that was created before (state: #{state})" do
        project = create(:project, state: state, project_type: :packaging, client: nil)
        measurements = create(:card_box_packaging_measurements, project: project)
  ...
  describe 'client' do
    describe 'can create `card-box` project' do
      it 'via filling form' do
        ...
        [:front_width, :front_height, :side_depth].each do |property|
          expect(measurements.public_send(property)).to eq('1.5 cm')
        end
      end
    ...
    [:waiting_for_style_details, :waiting_for_finish_details, :waiting_for_audience_details].each do |state|
      ...
      it "can see project that was created before (state: #{state})" do
        client = create(:client)
        login_as client.user
