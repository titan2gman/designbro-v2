...
GOD_CLIENT_PROJECT_STATES = Project.aasm.states.map(&:name).map(&:to_s)

GOD_CLIENT_BEFORE_PAYMENT_PROJECT_STATES = [
  Project::STATE_DRAFT,
...
GOD_CLIENT_NOT_BEFORE_PAYMENT_PROJECT_STATES = (
  GOD_CLIENT_PROJECT_STATES - GOD_CLIENT_BEFORE_PAYMENT_PROJECT_STATES
).map(&:to_s)

GOD_CLIENT_ALLOWED_UPDATE_PROJECT_STATES = [
  Project::STATE_WAITING_FOR_STYLE_DETAILS,
  Project::STATE_WAITING_FOR_AUDIENCE_DETAILS,
  Project::STATE_WAITING_FOR_FINISH_DETAILS,
  Project::STATE_WAITING_FOR_DETAILS,
...
GOD_CLIENT_NOT_ALLOWED_UPDATE_PROJECT_STATES = (
  GOD_CLIENT_PROJECT_STATES - GOD_CLIENT_ALLOWED_UPDATE_PROJECT_STATES
).map(&:to_s)

GOD_CLIENT_ALLOWED_DESTROY_PROJECT_STATES = [
  Project::STATE_WAITING_FOR_STYLE_DETAILS,
  Project::STATE_WAITING_FOR_AUDIENCE_DETAILS,
  Project::STATE_WAITING_FOR_FINISH_DETAILS
].map(&:to_s)

GOD_CLIENT_NOT_ALLOWED_DESTROY_PROJECT_STATES = (
  GOD_CLIENT_PROJECT_STATES - GOD_CLIENT_ALLOWED_DESTROY_PROJECT_STATES
).map(&:to_s)
...
RSpec.describe Ability do
  describe 'god client' do
    ...
    context 'project source file' do
      it { is_expected.to be_able_to :read, ProjectSourceFile }
    ...
    context 'direct conversation message' do
      let(:direct_conversation_message) { create(:direct_conversation_message) }

      it { is_expected.to be_able_to :read, direct_conversation_message }

      it { is_expected.not_to be_able_to :create,  direct_conversation_message }
      it { is_expected.not_to be_able_to :destroy, direct_conversation_message }
      it { is_expected.not_to be_able_to :update,  direct_conversation_message }
    end
    ...
    context 'project' do
      ...
      it 'can read all projects' do
        projects = [
          *create_list(:project, 2, client: client),
          *create_list(:project, 2)
        ]

        expect(Project.accessible_by(subject, :search)).to match_array(projects)
        expect(Project.accessible_by(subject, :read)).to match_array(projects)
      end

      it 'can update own projects with allowed states' do
        allowed_projects = GOD_CLIENT_ALLOWED_UPDATE_PROJECT_STATES
                           .map { |state| create(:project, state: state, client: client) }
        not_allowed_projects = GOD_CLIENT_NOT_ALLOWED_UPDATE_PROJECT_STATES
                               .map { |state| create(:project, state: state, client: client) }
      ...
      it 'can destroy own projects with allowed states' do
        allowed_projects = GOD_CLIENT_ALLOWED_DESTROY_PROJECT_STATES
                           .map { |state| create(:project, state: state, client: client) }
        not_allowed_projects = GOD_CLIENT_NOT_ALLOWED_DESTROY_PROJECT_STATES
                               .map { |state| create(:project, state: state, client: client) }
      ...
      context 'can read project of another client' do
        let(:project) { create(:project) }
    ...
    context 'project associations' do
      context 'public project states' do
        GOD_CLIENT_BEFORE_PAYMENT_PROJECT_STATES.each do |state|
          context state do
      ...
      context 'non public project states' do
        GOD_CLIENT_NOT_BEFORE_PAYMENT_PROJECT_STATES.each do |state|
          context state do
            let(:project) { create(:project, state: state, client: client) }
